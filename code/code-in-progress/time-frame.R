#determing climate sites with data missing from <10% of days in recorded time period and working on calculating precipitation, 
#average temperature, and temperature maximum metrics
...
clim.1<-with(clim,subset(clim,min.date<="1965-01-01"&max.date>="2014-12-31"))
...
clim.3<-map(clim.2, ~.x %>% filter(YEAR >= 1965, YEAR <= 2015))

clim.4 <- lapply(clim.3, function(dt)
tidyr::complete(dplyr::mutate(dt, DATE = as.Date(DATE)),DATE = seq(as.Date("1965-01-01"), as.Date("2014-12-31"), by = "day")))
...
temp<-with(clim.5,subset(clim.5,data.miss.tavg<=0.1,select = c("station_id","n_rows","data.miss.tmax","data.miss.tmin")))
...
temp.max.3<-rbindlist(lapply(names(temp.max.2.4),function(j){
	...
	x.low.min = quantile(x$TMIN,c(.05),na.rm=TRUE)
)}))
