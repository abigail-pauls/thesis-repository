#calculating climate variables for all soil data
...
tmin<-read.csv("/disks/home/abigail/thesis-repository/code/data/tmin.csv")
...
final$row_id<-as.integer(soil$row_id)

tmax.metric.1$row_id<-as.numeric(tmax.metric.1$row_id)
tmin.metric.1$row_id<-as.numeric(tmin.metric.1$row_id)
prcp.metric.1$row_id<-as.numeric(prcp.metric.1$row_id)

metrics<-merge(tmax.metric.1,tmin.metric.1,all=TRUE)
metrics<-merge(metrics,prcp.metric.1,all=TRUE)
soil<-merge(final,metrics,all.x=FALSE,all.y=TRUE)
...
write.csv(soil,"/disks/home/abigail/thesis-repository/code/data/final.csv")
