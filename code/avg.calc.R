...
tmax.0<-with(clim.5,subset(clim.5,data.miss.tmax<=0.1,select = c("station_id","n_rows","data.miss.tmax")))
...
avg.tmax<-data.table(station_id=character(),avg.tmax=numeric())

for(i in names(tmax.2.3)){
	df<-tmax.2.3[[i]]
	avg.tmax<-rbind(avg.tmax,data.table(
	station_id=i,
	avg.tmax=mean(df$TMAX,na.rm=TRUE)))}
...
tmin.0<-with(clim.5,subset(clim.5,data.miss.tmin<=0.1,select = c("station_id","n_rows","data.miss.tmin")))
...
avg.tmin<-data.table(station_id=character(),avg.tmin=numeric())

for(i in names(tmin.2.3)){
	df<-tmin.2.3[[i]]
	avg.tmin<-rbind(avg.tmin,data.table(
	station_id=i,
	avg.tmin=mean(df$TMIN,na.rm=TRUE)))}
