...
for(cc in soil.n){
	...
	df$climate<-as.numeric(df$slope)
if(nrow(df)<3) next
if(length(levels(df$biome))<2)next
	model<-lm(slope~soil*biome,df)
name<-paste0(ss,sep=".",cc)
nitkjd.stats[[name]]<-as.data.frame(summary(model)$coefficients)}}
...
if(length(levels(df$biome))<2)next
        model<-lm(slope~soil*biome,df)
name<-paste0(ss,sep=".",cc)
orgc.stats[[name]]<-as.data.frame(summary(model)$coefficients)}}

install.packages(emmeans)
library(emmeans)

test<-emmeans(model,~biome)
