...
anova<-data.frame(slope=character(),df=numeric(),sum.sqr=numeric(),mean.sqr=numeric(),f.value=numeric(),p.value=numeric())
...
if(length(levels(df$biome))<2) next
	...
	a<-summary(model)[[1]]
anova<-rbind(anova,data.frame(slope=s,df=a$Df[1],sum.sqr=a$`Sum Sq`[1],mean.sqr=a$`Mean Sq`[1],f.value=a$`F value`[1],p.value=a$`Pr(>F)`[1]))}
