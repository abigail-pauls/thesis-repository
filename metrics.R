...
for(s in y){
	...
	for(i in rows){	
		...
		time<-x$DATE<=soil.all$date[i]&x$DATE>=soil.all$start.date[i]
			...
			for(j in 1:365){
				roll<-frollsum(x$PRCP,n=j,align="right")
				dry_roll<-roll[x$PRCP==0]
				soil.all$a.[[j]][i]<-sum(roll.na.rm=TRUE)
				soil.all$b.[[j]][i]<-sum(dry_roll,na.rm=TRUE)
}}}
