...
temp.max<-with(clim.5,subset(clim.5,data.miss.tmax<=0.1,select=c("station_id","n_rows","data.miss.tmax")))
...
temp<-merge(temp.max,temp.min,by="station_id",all=TRUE)
metric<-merge(temp,prcp,by="station_id",all=TRUE)

soil<-read.csv("/disks/home/abigail/thesis-repository/lp.clim.csv")

soil[,c("tecq","orgm","orgc","totc","nitkjd","ecec","cfgr","cfvo","clay","silt","sand","phaq","phetol","wg1500")]<-NULL

soil.all<-merge(soil,metric,by="station_id",all.x=FALSE,all.y=TRUE)
