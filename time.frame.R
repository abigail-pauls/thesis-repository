...
precip.2.2<-map(precip.2.1, ~.x %>% filter(YEAR >= 1973, YEAR <= 2023))

precip.2.3 <- lapply(precip.2.2, function(dt)
tidyr::complete(dplyr::mutate(dt, DATE = as.Date(DATE)),DATE = seq(as.Date("1973-01-01"), as.Date("2023-12-31"), by = "day")))

precip.2.4<- lapply(precip.2.3,function(b){b$PRCP<-as.numeric(b$PRCP)/10})

map.0 <- lapply(precip.2.4, function(d) {
aggregate(PRCP ~ YEAR*STATION, data = d, sum, na.rm = TRUE)})

map.1<-rbindlist(lapply(names(map.0),function(i){
x<-map.0[[i]]
data.table(station_id = basename(i),map = with(x,aggregate(PRCP~STATION,data=x,mean,na.rm=TRUE)))}))

precip.3<-rbindlist(lapply(names(precip.2.4),function(j){
	x<-precip.2.4[[j]]
	data.table(
